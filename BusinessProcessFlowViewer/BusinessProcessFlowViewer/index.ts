...
export class BusinessProcessFlowViewer implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	 ...
	 */
	private async processAllRecordsInDataSet(jsonParametersBPF: BusinessProcessFlowJson) {
		...
		try {
			...
			if(result && result.entities.length>0)
			{
				result.entities.forEach(item => {
					var stagename:string = item["stagename"]; //stagecategory@OData.Community.Display.V1.FormattedValue
	...
	private initializeParameters() {
		...
		this._pulseColor = this._context.parameters.pulseColor.raw ? this._context.parameters.pulseColor.raw : "" ;
			this._displayEntityName = this._context.parameters.displayEntityName != null? (this._context.parameters.displayEntityName.raw == "true" ? true : false) : false;
	}
	/**
